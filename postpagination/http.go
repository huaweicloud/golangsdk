...
func Request(client *golangsdk.ServiceClient, method string, request interface{}, headers map[string]string, url string) (*http.Response, error) {
	if method == "POST" {
		return client.Post(url, request, nil, &golangsdk.RequestOpts{
			MoreHeaders: headers,
